...
struct SkeletonCategoryRow: View {
    var body: some View {
        HStack(spacing: 14) {
            RoundedRectangle(cornerRadius: 5).fill(Color.white.opacity(0.10)).frame(width: 18, height: 18)
            RoundedRectangle(cornerRadius: 8).fill(Color.white.opacity(0.10)).frame(width: 38, height: 38)
            VStack(alignment: .leading, spacing: 6) {
                RoundedRectangle(cornerRadius: 4).fill(Color.white.opacity(0.10)).frame(width: 160, height: 12)
                RoundedRectangle(cornerRadius: 4).fill(Color.white.opacity(0.06)).frame(width: 240, height: 9)
            }
            Spacer()
            RoundedRectangle(cornerRadius: 4).fill(Color.white.opacity(0.10)).frame(width: 70, height: 12)
        }
        .padding(16)
